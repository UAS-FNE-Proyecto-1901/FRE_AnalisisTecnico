...
```{r serieTiempoCreacion, echo=FALSE, fig.cap='Serie de tiempo de actas de creación de FRE.', fig.topcaption=TRUE, message=FALSE, warning=FALSE, fig.width=10, out.width="100%", fig.asp=0.5, fig.align='center', fig.pos='h'}
# fig.height=8,
...
```{r perfilProfesional3, echo=FALSE, fig.cap='N.° de personas que trabajan en el FRE. (A) Conteo de FRE por número de personas. (B) Mapa con número de personas por departamento.', fig.width=13, fig.height=5, fig.topcaption=TRUE, out.width="100%",fig.align='center',fig.pos='bh'}
...
```{r pieProfesional2, echo=FALSE, fig.cap='Tipo de vinculación al FRE.', fig.width=8, fig.height=4.5, fig.topcaption=TRUE, out.width="85%",fig.align='center',fig.pos='h'}
...
```{r institucionesInscritasVARdemo, echo=FALSE, fig.cap='Relación entre N.° de instituciones inscritas en el FRE y: (A) tamaño de la población en el departamento o (B) N.° de camas de acuerdo a REPS.', fig.width=10, fig.height=4, out.width="100%", fig.topcaption=TRUE, message=FALSE, warning=FALSE,fig.align='center',fig.pos='b'}
...
En la Figura \@ref(fig:IngresosFRE1) se observa que una tendencia a que la proporción de ingresos más grande es aquella que reciben los FRE departamentales por la venta de MME. Esto se puede deber a que cada departamento maneja independencia en los precios de venta de estos medicamentos y desde ahí se podría obtener una rentabilidad importante. En el desarrollo del apartado [6.4 Precio de medicamentos](#precioMedicamentos) del presente informe, se desglosará un análisis más profundo sobre los precios de venta de los MME por departamento, pero a *grosso modo* se puede notar que el margen es lo suficientemente alto para abarcar el porcentaje mayoritario de ingresos. 
...
```{r IngresosFRE2, echo=FALSE, warning=FALSE, fig.cap='Mapa con fuentes de ingresos de los FRE.', fig.width=8, fig.height=6, fig.topcaption=TRUE, out.width="85%", message=FALSE, fig.align='center',fig.pos='b'}
readRDS(file.path(fig_path, "136_IngresosFRE2.rds")) +labs(title = NULL)
```

## Clasificación de los FRE por nivel de complejidad
...
```{r PCA1, echo=FALSE, warning=FALSE, fig.cap='Variables involucradas en la construcción de los componentes principales', fig.width=6, fig.asp=0.5, fig.topcaption=TRUE, out.width="90%", message=FALSE,fig.align='center',fig.pos='t'}
fig_path <- file.path('..', '..', 'figures')
...
Se realizó un análisis de agrupación (*cluster*) jerárquico mediante el algoritmo de Ward en el software R. Se seleccionaron 5 grupos de acuerdo al análisis mediante un criterio de *codo*. A partir de la descripción anterior, la clasificación según la complejidad de los FRE, resultó como aparece en la Figura \@ref(fig:PCA1-2).

```{r PCA1-2, echo=FALSE, warning=FALSE, fig.cap='Clasificación de los FRE en grupos de acuerdo a análisis por clúster', fig.width=8, fig.asp=0.75, fig.topcaption=TRUE, out.width="90%", message=FALSE,fig.align='center',fig.pos='t'}
g1 <- readRDS(file.path(fig_path, '011_Clasificacion', "013_cluz_group.rds")) + labs(title = NULL)
g1
```
Luego de un análisis multivariado en el software R, se definieron varios parámetros a tomar en cuenta para esta clasificación. Según los datos obtenidos por las 5 variables en cada departamento, se lograron agrupar los FRE en 3 categorías, las cuales serán descritas a continuación:
